...
public class NoticeView extends TextSwitcher
{
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
    {
        ...
        if (mIcon != null)
        {
            ...
            mIcon.setBounds(mPaddingLeft, y, mPaddingLeft + mIcon.getIntrinsicWidth(), y + mIcon
                    .getIntrinsicHeight());
        }
    }
    ...
    private void update()
    {
        ...
        OLLogger.e("NoticeView", "mIsVisible " + mIsVisible + " mIsresumed " + mIsResumed + " " +
                "mIsStarted " + mIsStarted + "mIsRunning " + mIsRunning);
        if (running != mIsRunning)
        {
            if (running)
            {
                OLLogger.e("NoticeView", "mDataList " + mDataList.size());
                if (mDataList.size() != 1)
                {
                    postDelayed(mRunnable, mInterval);
                }
    ...
    private void show(int index)
    {
        ...
        String s = mDataList.get(mIndex);
        String[] res = s.split(OneLotteryApi.RES_SEP);
        setText(Html.fromHtml(res[0]));
    }

    private Animation anim(float from, float to)
    {
        final TranslateAnimation anim = new TranslateAnimation(0, 0f, 0, 0f, Animation
                .RELATIVE_TO_PARENT, from, Animation.RELATIVE_TO_PARENT, to);
