...
{
    public CardEditText(Context ctx)
    {
        this(ctx, null);
    }
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count)
        {
            ...
            if (isMiddle || isNeedSpace)
            {
                ...
                for (int i = 0; i < string.length(); i += 4)
                {
                    ...
                    {
                        sb.append(string.substring(i, i + 4));
                    } else
                    {
                ...
                } else if (isMiddle)
                {
                    ...
                    if (count == 0)
                    {
                        ...
                        {
                            setSelection((start - before + 1) > sb.length() ? sb.length() :
                                    (start - before + 1));
                        }
                    }
                    //如果是增加
                    else
                    {
                        ...
                        {
                            setSelection((start + count - before + 1) < sb.length() ? (start +
                                    count - before + 1) : sb.length());
                        } else
                        {
