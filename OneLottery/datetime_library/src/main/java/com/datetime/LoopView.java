...
//        paintB.setTextScaleX(1.05F);
    ...
    private void e() {
        ...
        for (int i1 = 0; i1 < arrayList.size(); i1++) {
            ...
            paintB.getTextBounds(s1, 0, s1.length(), rect2);
            // wangpeng:调整可触摸区域的宽度为2个字符
            paintB.getTextBounds("00", 0, "00".length(), rect);
            int j1 = rect.width();
            // wangpeng:调整区域增大2倍，提升体验。
            j1 = (int) (j1 * 2.0f);

//            android.util.Log.i("ddd", "arr["+i1+"]=" + arrayList.get(i1) + ", width="+g+", j1=" + j1);
            if (j1 > g) {
                g = j1;
            }
            // 如果实际字符宽比较宽，采用此宽度
...
//                    android.util.Log.i("ddd", str+" width= " + itemWidth + ", maxWidth="+maxWidth + ", startX="+startX);
