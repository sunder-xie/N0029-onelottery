 ...
 */
process.env.GRPC_SSL_CIPHER_SUITES = process.env.GRPC_SSL_CIPHER_SUITES ? process.env.GRPC_SSL_CIPHER_SUITES : 'ECDHE-RSA-AES128-GCM-SHA256:' +
...
var Member = (function() {
    ...
    Member.prototype.invoke = function(invokeRequest) {
        debug("Member.invoke");
        var tx = this.newTransactionContext(invokeRequest.userCert);
    ...
    Member.prototype.query = function(queryRequest) {
        debug("Member.query");
        var tx = this.newTransactionContext(queryRequest.userCert);
        ...
        self.getMyTCert(function(err, tcert) {
            ...
            if (tcert) {
                ...
                if (tx.pb.getConfidentialityLevel() == _fabricProto.ConfidentialityLevel.CONFIDENTIAL &&
                    ...
                    var eventhub = global.event;
                    var cbe = event.registerChaincodeEvent(tx.chaincodeID, tx.fcn, function(event) {
                        if (event.txID === uuid_1) {
                            self.complete = true;
                            if (self.timeoutId) {
                                clearTimeout(self.timeoutId);
                            }
                            eventhub.unregisterChaincodeEvent(cbe);
                            self.emit("complete", JSON.parse(event.payload.toString()));
                        }
                    });
                        ...
                        self.timeoutId = setTimeout(function() {
                            ...
                            // emit a complete event as done previously
                                if (event.isconnected()) {
                                    ...
                                    self.emit("error", "the server disconnected");
                                    eventhub.unregisterChaincodeEvent(cbe);
                                    global.event.connect();
                                }
                            else
                                eventhub.unregisterChaincodeEvent(cbe);
                        }, self.waitTime * 1000);
...
var MemberServicesImpl = (function() {
    ...
    MemberServicesImpl.prototype.register = function(req, registrar, cb) {
        ...
        protoRegistrar.setId({
            id: registrar.getName()
        });
        if (req.registrar) {
    ...
    MemberServicesImpl.prototype.enroll = function(req, cb) {
        ...
        eCertCreateRequest.setId({
            id: req.enrollmentID
        });
        eCertCreateRequest.setTok({
            tok: new Buffer(req.enrollmentSecret)
        });
